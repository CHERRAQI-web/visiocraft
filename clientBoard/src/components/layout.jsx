...
import { Menu, Avatar, Text, Group, UnstyledButton, Loader, Alert } from "@mantine/core";
import { IconChevronDown, IconLogout, IconDashboard, IconBook, IconAlertCircle, IconMenu2, IconX } from "@tabler/icons-react";
import './Sidebar.css';
...
const Layout = () => {
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-slate-100">
        <div className="max-w-md w-full p-6">
          <Alert icon={<IconAlertCircle size={16} />} title="Access Denied" color="red" radius="md">
            {error || "You are not authenticated. Please log in to access this page."}
          </Alert>
          <div className="mt-4 text-center">
            <button onClick={handleLogout} className="px-4 py-2 bg-violet-600 text-white rounded-md hover:bg-violet-700 transition-colors">
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: 'var(--main-bg)' }}>
      ...
      {/* Sidebar */}
      <div className={`bg-sky-600 text-slate-300 transition-all duration-300 ease-in-out fixed md:sticky top-0 h-screen z-50 flex flex-col justify-between ${
        isSidebarOpen ? (isMobile ? "w-64 left-0" : "w-64 left-0") : (isMobile ? "w-64 -left-64" : "w-20 left-0")
      }`}>
        <div className="flex flex-col flex-1">
          <div className="flex justify-between items-center p-4">
            ...
            >
              {isMobile ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 transform transition-transform duration-300 ${isSidebarOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <ul className="px-3 mt-6">
            ...
            <SidebarLink to="/projects" icon={IconBook} label="My Projects" />
          </ul>
        </div>
      ...
      {/* Main Content */}
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isMobile ? "ml-0" : (isSidebarOpen ? "ml-0" : "ml-0")}`}>
        <header className="bg-sky-600 flex items-center justify-between sticky shadow-sm border-b border-slate-200 top-0 z-10 px-6">
          ...
          <div className="px-9 py-4">
            <h1 className="text-xl font-semibold text-white">Hello, {getUserName()}</h1>
          </div>
            ...
            <Menu.Target>
              <UnstyledButton className="flex items-center space-x-2 sm:space-x-4 hover:bg-sky-500 rounded-lg transition-all duration-300 p-1 transform hover:scale-105">
                <Group className="flex items-center space-x-2 sm:space-x-4">
                  <Avatar radius="xl" style={{backgroundColor:"#81E6D9", text:"white"}} className="w-10 h-10 flex items-center justify-center font-bold text-white hover:scale-110 transition-transform duration-200">
                    {getUserInitials()}
                  </Avatar>
                  {!isMobile && (
                    <div>
                      <Text size="xs" style={{color:"white"}} className="text-teal-200">{getUserEmail()}</Text>
                    </div>
                  )}
                </Group>
              </UnstyledButton>
